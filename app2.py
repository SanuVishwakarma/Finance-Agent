...
def initialize_agents():
    ...
    if not st.session_state.agents_initialized:
        try:
            llm = Groq(api_key=GROQ_API_KEY, id="llama-3.3-70b-versatile")
            ...
            st.session_state.web_agent = Agent(
                ...
                instructions=[
                    ...
                    "Provide verified information from reliable sources",
            ...
            st.session_state.finance_agent = Agent(
                ...
                role="Analyze financial data and provide market insights",
                model=llm,
                tools=[YFinanceTools()],
                instructions=[
            ...
            st.session_state.multi_ai_agent = Agent(
                name="Multi-Source Analysis Agent",
                role="Provide comprehensive market analysis",
                model=llm,
                team=[st.session_state.web_agent, st.session_state.finance_agent],
                instructions=[
                    "Combine data from multiple sources",
                    "Provide detailed analysis with supporting data",
                    "Include both technical and fundamental insights"
                ],
                markdown=True
            )
...
def get_agent_response(agent, query, retries=3):
    """Helper function to get response from agent with retries"""
...
def format_agent_response(response):
    """Helper function to format agent responses for Streamlit"""
    if not response:
        return ""
    # Clean up response text
    response = response.strip()
    # Convert markdown-style links to HTML
    response = re.sub(r'\[(.*?)\]\((.*?)\)', r'<a href="\2">\1</a>', response)
    return response
...
def main():
    ...
    # Main content
    # st.markdown('<h1 class="stock-header">🤖 Advanced Stock Market Analysis By </h1>', 
    #             unsafe_allow_html=True)
                            ...
                            with analysis_tab:
                                ...
                                try:
                                    with st.spinner("Generating AI analysis..."):
                                        # Create analysis query based on selected type
                                        if analysis_type == "Comprehensive Analysis":
                                            query = f"""Please provide a comprehensive analysis for {stock_symbol} including:
                                            - Recent performance analysis
                                            - Key financial metrics
                                            - Main business developments
                                            """
                                        elif analysis_type == "Technical Analysis":
                                            query = f"""Provide technical analysis for {stock_symbol} including:
                                            - Price and volume patterns
                                            - Key technical indicators
                                            - Support and resistance levels
                                            """
                                        elif analysis_type == "Fundamental Analysis":
                                            query = f"""Analyze fundamental aspects of {stock_symbol} including:
                                            - Key fundamental metrics
                                            - Industry comparison
                                            - Growth prospects
                                            """
                                        elif analysis_type == "News Analysis":
                                            query = f"""Analyze recent news impact on {stock_symbol} including:
                                            - Major news events
                                            - Price impact of news
                                            - Market sentiment from news
                                            """
                                        else:  # Sentiment Analysis
                                            query = f"""Analyze market sentiment for {stock_symbol} including:
                                        ...
                                        try:
                                            # Use the finance agent for analysis
                                            response = st.session_state.finance_agent.print_response(query, stream=False)
                                            st.markdown(response) if response else st.warning("No analysis data available at this time.")
                                            
                                        except Exception as query_error:
                                            st.error(f"Error generating analysis: {str(query_error)}")
                            ...
                            with news_tab:
                                ...
                                try:
                                    with st.spinner("Fetching latest news..."):
                                        # Get news using finance agent
                                        news_query = f"Get the latest important news articles about {stock_symbol}"
                                        
                                        try:
                                            # Try getting news from finance agent
                                            finance_news = st.session_state.finance_agent.print_response(news_query, stream=False)
                                            if finance_news:
                                                st.markdown("#### Recent News Analysis")
                                                st.markdown(finance_news)
                                                st.markdown("---")
                                            
                                            # Display yfinance news if available
                                            if info and 'news' in info and info['news']:
                                                st.markdown("### Company News Feed")
                                                    ...
                                                    with st.container():
                                                        ...
                                                        <div class="news-card">
                                                            ...
                                                            <small>
                                                                ...
                                                                {datetime.fromtimestamp(news_item.get('providerPublishTime', datetime.now().timestamp())).strftime('%Y-%m-%d %H:%M:%S')}
                                                            </small>
                                                        </div>
                                                        """, unsafe_allow_html=True)
                                            
                                            # If no news is available from either source
                                            if not (finance_news or (info and 'news' in info and info['news'])):
                                                st.warning("No recent news available at this time.")
                                                
                                        except Exception as news_error:
                                            st.error(f"Error fetching news data: {str(news_error)}")
                                            
                                except Exception as e:
                                    st.error(f"Error in news tab: {str(e)}")
                                    st.markdown("Please try refreshing the page or try again later.")
